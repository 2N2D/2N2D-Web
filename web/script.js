...
console.log("Available Eel functions:", Object.keys(eel).filter(k => !k.startsWith('_')));
...
document.addEventListener('DOMContentLoaded', function() {
    console.log("DOM loaded, testing Eel connection");
    setTimeout(testEelConnection, 1000); 
});


document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM loaded, exposing functions to Eel');
    
    
    if (typeof eel !== 'undefined') {
        
        eel.expose(updateOptimizationProgress);
        console.log('Progress function exposed to Eel');
    } else {
        console.error('Eel is not defined! Check if /eel.js is loaded');
    }
    
});


function ensureProperTabVisibility() {
    
    const activeTabButton = document.querySelector('.tab-button.active');
    
    if (activeTabButton) {
        
        const contentId = 'content-' + activeTabButton.id.replace('tab-', '');
        
        
        document.querySelectorAll('.tab-pane').forEach(pane => {
            if (pane.id === contentId) {
...
setTimeout(ensureProperTabVisibility, 100);
...
function switchToTab(tabId) {
    ...
    if (tabId === 'tab-optimize') {
        setTimeout(initializeOptimizationButton, 100); 
    }
}
...
async function startOptimization() {
    ...
    try {
        ...
        const result = await eel.find_optimal_architecture(selectedInputs, selectedTarget, epochs)();
...
function testEelConnection() {
    console.log("Testing Eel connection...");
    
    
    if (typeof eel.test_connection === 'function') {
        eel.test_connection()().then(response => {
            console.log("Eel connection test successful:", response);
        }).catch(err => {
            console.error("Eel connection test failed:", err);
        });
    } else {
        console.warn("eel.test_connection is not available");
    }
}
...
function displayOptimizationResults(results) {
    ...
    document.getElementById('best-loss').textContent = bestConfig.test_loss.toFixed(6);
    document.getElementById('best-r2').textContent = bestConfig.r2_score.toFixed(4);
}
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            const item = dataByLayers[context.dataset.label.split(' ')[0]][context.dataIndex];
...
async function downloadOptimizedModel() {
    ...
    try {
        ...
        const result = await window.eel.download_optimized_model(window.optimizedModelPath)();
...
async function runModelTest(testSizeSlider) {
    ...
    const inputFeatures = Array.from(inputFeaturesSelect.selectedOptions).map(option => option.value);
    ...
    try {
        ...
        const result = await window.eel.test_model(inputFeatures, targetFeature, testSize)();
...
function displayTestResults(results) {
    ...
    if (results.metrics) {
        ...
        document.getElementById('metric-mse').textContent = formatNumber(results.metrics.mse);
        document.getElementById('metric-mae').textContent = formatNumber(results.metrics.mae);
        document.getElementById('metric-r2').textContent = formatNumber(results.metrics.r2);
        document.getElementById('metric-samples').textContent = results.metrics.samples;
    }
...
function updateDataSummary(summary) {
    ...
    const totalMissing = summary.missing_values ? 
        Object.values(summary.missing_values).reduce((sum, val) => sum + val, 0) : 0;
...
function createNetworkVisualization(graphData) {
    try {
        ...
        if (!graphData || !graphData.nodes || !graphData.edges) {
            container.innerHTML = '<div class="error-message">No valid graph data available</div>';
            return;
        }
        
        console.log(`Creating 2D visualization with ${graphData.nodes.length} nodes`);
...
function calculateNodePositions(nodes, edges) {
    ...
    if (Object.keys(layers).length <= 1) {
        ...
        const nodesToProcess = nodes.filter(node => !incomingEdges[node.id]).map(node => node.id);
...
function createNodes(nodes, positions) {
    ...
    const typeColors = {};
    const colors = [
        0x4f46e5, 0x7c3aed, 0xe11d48, 0xf59e0b, 
        0x10b981, 0x3b82f6, 0xec4899, 0x8b5cf6
    ];
    ...
    nodes.forEach(node => {
        ...
        const color = node.color ? parseInt(node.color.replace('#', '0x')) : 
...
async function handleCsvUpload(event) {
    ...
    try {
        ...
        updateStatus(`Dataset loaded: ${file.name} (${result.summary.rows} rows, ${result.summary.columns} columns)`);
    ...
    function displayLayersInfo(layers) {
        ...
        if (!layers || layers.length === 0) {
            layersTableBody.innerHTML = '<tr><td colspan="3">No layer information available</td></tr>';
            return;
        }
    ...
    async function getArchitectureSuggestions() {
        try {
            ...
            document.getElementById('architecture-visualization').classList.remove('hidden');
        ...
        architectureChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: [
                        'rgba(54, 162, 235, 0.7)',
                        ...Array(architecture.hidden_layers.length).fill('rgba(75, 192, 192, 0.7)'),
                        'rgba(255, 99, 132, 0.7)'
                    ],
                    borderColor: [
                        'rgba(54, 162, 235, 1)',
                        ...Array(architecture.hidden_layers.length).fill('rgba(75, 192, 192, 1)'),
                        'rgba(255, 99, 132, 1)'
                    ],
                    borderWidth: 1
                }]
...
document.getElementById('debugBtn').addEventListener('click', function() {
    ...
    if (tabButton) {
        ...
        setTimeout(() => {
            ...
            document.getElementById('content-optimize').style = 'display: block !important;';
