...
const OneTimeMailSignInButton = () => {
    ...
    async function sendMail(e: FormEvent<HTMLFormElement>) {
        ...
        if (email) {
            ...
            await sendSignInLinkToEmail(getAuth(initFirebaseApp()), email, actionCodeSettings);
        }

    }

    if (!state) {
        return <button className={Styles.button} onClick={() => setState(true)}><i
            className="fa-solid fa-envelope"></i> Magic Link</button>
    } else {
